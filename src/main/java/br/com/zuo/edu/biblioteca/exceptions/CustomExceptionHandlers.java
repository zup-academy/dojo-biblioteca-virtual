...
public class CustomExceptionHandlers {
    ...
    public ResponseEntity<ErroPadronizado> handleMethodArgumentNotValid(MethodArgumentNotValidException ex) {
        ...
        ErroPadronizado erroPadronizado = new ErroPadronizado(codigoHttp,mensagemHttp,mensagemGeral,mensagens);

        return ResponseEntity.badRequest().body(erroPadronizado);
    }

    @ExceptionHandler(ObjectOptimisticLockingFailureException.class)
    public ResponseEntity<ErroPadronizado> handleObjectOptimisticLockingFailureException(ObjectOptimisticLockingFailureException ex) {
        HttpStatus badRequestStatus = HttpStatus.BAD_REQUEST;
        Integer codigoHttp = badRequestStatus.value();
        String mensagemHttp = badRequestStatus.getReasonPhrase();
        String mensagemGeral = "Houve um erro.";
        List<String> mensagens = new ArrayList<>();

        mensagens.add("Recurso que tentou atualizar mudou de estado");
